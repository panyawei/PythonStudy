...
class MovieContainer(object):
    ...
    def saveQDataStream(self):
        ...
        try:
            ...
            error = "Failed to save: {0}".format(err)
        finally:
            if fh is not None:
                fh.close()
    ...
    def saveQTextStream(self):
        ...
        try:
            ...
            for key, movie in self.__movies:
                ...
                stream << movie.year << " " << movie.minutes << " "
                stream << movie.acquired.toString(Qt.ISODate)
                stream << "\n{NOTES}"
    ...
    def loadQTextStream(self):
        ...
        try:
            ...
            error = "Failed to load: {0} on line {1}".format(err, lino)
        finally:
            if fh is not None:
                fh.close()
            if error is not None:
        ...
        try:
            ...
            for key, movie in self.__movies:
                stream << ("<MOVIE YEAR='{0}' MINUTES='{1}' "
                           ...
                           movie.minutes,
                           movie.acquired.toString(Qt.ISODate)))
                stream << "<TITLE>" << Qt.escape(movie.title)
                stream << "</TITLE>\n<NOTES>"
                if not movie.notes.isEmpty():
                    stream << "\n" << Qt.escape(
            ...
            error = "Failed to export: {0}".format(err)
        finally:
            if fh is not None:
                fh.close()
            if error is not None:
    ...
    def importDOM(self, fname):
        ...
        try:
            ...
            error = "Failed to import: {0}".format(err)
        finally:
            if fh is not None:
                fh.close()
            if error is not None:
        ...
        try:
            ...
            return False, "Failed to import: {0}".format(err)
        self.__fname = QString()
        self.__dirty = True
